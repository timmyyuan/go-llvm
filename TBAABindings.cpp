                                     ...
                                     bool &MayAlias)
{
    ...
    for (;;)
    {
        ...
        if (BaseType.getNode() == SubobjectTag.getBaseType())
        {
            ...
            if (GenericTag)
            {
                *GenericTag = SameMemberAccess ? SubobjectTag.getNode() : createAccessTag(CommonType);
            }
...
{
    AAMDNodes A = unwrap<Instruction>(LhsInst)->getAAMetadata();
    AAMDNodes B = unwrap<Instruction>(RhsInst)->getAAMetadata();

    return matchAccessTags(A.TBAA, B.TBAA);
}